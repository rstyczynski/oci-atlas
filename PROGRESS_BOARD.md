# Progress board

Progress board is a table showing sprint, and backlog items state. It's the only purpose of this file. All potential comments, progress notes, etc. always keep in dedicated files for each phase.

| Sprint   | Sprint Status       | Backlog Item | Item Status         |
| -------- | ------------------- | ------------ | ------------------- |
| Sprint 1 | done                | GD-1         | done                |
| Sprint 2 | failed              | GD-1-fix1    | rejected            |
| Sprint 3 | done                | GD-2         | done                |
| Sprint 4 | done                | GD-4         | tested              |
| Sprint 5 | done                | GD-5         | done                |
| Sprint 6 | implemented         | GD-6         | tested              |
| Sprint 7 | implemented         | GD-10        | tested              |
| Sprint 8 | implemented         | GD-18        | tested              |
